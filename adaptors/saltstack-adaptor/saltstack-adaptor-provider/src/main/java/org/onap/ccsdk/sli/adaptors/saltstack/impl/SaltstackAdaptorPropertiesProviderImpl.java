...
public class SaltstackAdaptorPropertiesProviderImpl implements SaltstackAdaptorPropertiesProvider {
     ...
     */
    private static final String SALTSTACKAdaptor_PROP_FILE_NAME = "saltstack-adaptor.properties";
     ...
     * files.
     */
    public SaltstackAdaptorPropertiesProviderImpl() {
        saltstackAdaptorPropertiesFileResolvers
        ...
        saltstackAdaptorPropertiesFileResolvers.add(
                new JREFileResolver("Using property file (3) from JRE argument", SaltstackAdaptorPropertiesProviderImpl.class));
        saltstackAdaptorPropertiesFileResolvers.add(new KarafRootFileResolver("Using property file (4) from karaf root", this));
        ...
        if (propertiesFile != null) {
            ...
            InputStream propStr = getClass().getResourceAsStream("/" + SALTSTACKAdaptor_PROP_FILE_NAME);
            if (propStr != null) {
                properties = new Properties();
                try {
        ...
        if (properties == null) {
            reportFailure("Missing configuration properties resource(3)", new ConfigurationException(
                    "Missing configuration properties resource(3): " + SALTSTACKAdaptor_PROP_FILE_NAME));

            LOG.info("Defaulting org.onap.appc.adaptor.saltstack.clientType to NONE");

            properties = new Properties();
            properties.setProperty("org.onap.appc.adaptor.saltstack.clientType", "NONE");
        }
    }
     ...
     * </ol>
     */
    File determinePropertiesFile(final SaltstackAdaptorPropertiesProviderImpl resourceProvider) {

        for (final PropertiesFileResolver sliPropertiesFileResolver : saltstackAdaptorPropertiesFileResolvers) {
            final Optional<File> fileOptional = sliPropertiesFileResolver.resolveFile(SALTSTACKAdaptor_PROP_FILE_NAME);
            if (fileOptional.isPresent()) {

        ...
        if (classHasSpecialHandling(simpleTypeName)) {
            try {
                ...
                if (retValue != null) {
                    ...
                    MdsalHelper.setProperty(props, pfx, propVal);
                }
            } catch (Exception e) {
            ...
            MdsalHelper.setProperty(props, pfx + "_length", Integer.toString(fromList.size()));
        } else if (fromClass.isEnum())
        {
            try {
                if (useLegacyEnumerationMapping) {
                    ...
                    MdsalHelper.setProperty(props, pfx, mapEnumeratedValue(pfx, propVal)));
                } else {
                    Method method = fromClass.getMethod("getName");
                    String yangValue = (String) method.invoke(fromObj);
                    MdsalHelper.setProperty(props, pfx, yangValue);
                }
            } catch (Exception e) {
                LOG.error("Caught exception trying to convert value returned by " + fromClass.getName()
                        + ".getValue() to Properties entry", e);
                    ...
                    if (isYangGenerated(returnType)) {
                            ...
                            try {
                                ...
                                if (retValue != null) {
                                    ...
                                    if (useLegacyEnumerationMapping) {
                                        ...
                                        MdsalHelper.setProperty(props, propName, mapEnumeratedValue(fieldName, propVal));
                                    } else {
                                        Method method = retValue.getClass().getMethod("getName");
                                        String yangValue = (String) method.invoke(retValue);
                                        MdsalHelper.setProperty(props, propName, yangValue);
                                    }
                        ...
                        try {
                            ...
                            if (propValObj != null) {
                                ...
                                MdsalHelper.setProperty(props, propName, propVal);
                            }
                        } catch (Exception e) {
                            if (m.getName().equals("getKey")) {
                                LOG.trace("Caught " + e.getClass().getName()
            ...
            if ((numGetters == 1) && ("getValue".equals(lastGetterName)) || isYangScalarType(fromObj)) {// || isYangTypeObject(fromObj)) {
            	MdsalHelper.setProperty(props, propNamePfx, propVal);
            }
        } else {
            ...
            MdsalHelper.setProperty(props, pfx, fromVal);
        }
